...
public:
  ...
  bool loadFromBinary(const std::string &name);
...
template<class TDescriptor, class F>
bool TemplatedVocabulary<TDescriptor,F>::loadFromBinary(const std::string &name)
{
    std::fstream fvoc(name, std::ios::in | std::ios::binary | std::ios::ate);
    if (!fvoc.is_open()) {
        std::cerr << "Vocabulary file " << name << " open failed" << std::endl;
        return false;
    }

    auto fsize = fvoc.tellg();
    fvoc.seekg(0);

    m_nodes.clear();
    m_words.clear();

    uint8_t k = 0, L = 0, s = 0, w = 0;
    fvoc.read((char*) &k, 1);
    fvoc.read((char*) &L, 1);
    fvoc.read((char*) &s, 1);
    fvoc.read((char*) &w, 1);
    if (k > 20 || L < 1 || L > 10 || s > 5 || w > 3) {
        std::cerr << "Invalid vocabulary header (k=" << (int) k << " L=" << (int) L << " s=" << (int) s << " w=" << (int) w << ")" << std::endl;
        fvoc.close();
        return false;
    }
    ...
    m_k = k;
    m_L = L;
    m_scoring = static_cast<ScoringType>(s);
    m_weighting = static_cast<WeightingType>(w);

    createScoringObject();

    int expected_nodes = static_cast<int>((std::pow(m_k, m_L + 1.) - 1.) / (m_k - 1.));
    m_nodes.reserve(expected_nodes);
    m_nodes.resize(1);
    m_nodes[0].id = 0;
    m_words.reserve(std::pow(m_k, m_L + 1.));

    while (!fvoc.eof() && fvoc.tellg() != fsize) {
        uint32_t pid = 0;
        uint8_t isLeaf = 0;
        fvoc.read((char*) &pid, 4);
        fvoc.read((char*) &isLeaf, 1);
        ...
        uint8_t desc[F::L] = {0};
        fvoc.read((char*) desc, F::L);
        F::fromArray(m_nodes[nid].descriptor, desc);

        float weight = 0.f;
        fvoc.read((char*) &weight, 4);
        m_nodes[nid].weight = weight;

        if (isLeaf) {
            auto wid = m_words.size();
