                ...
                for(size_t iKF=0; iKF<vIndicesKF.size(); iKF++)
                {
                    ...
                    for(size_t iF=0; iF<vIndicesF.size(); iF++)
                    {
                        if(F.Nleft == -1){
                            ...
                            if(realIdxF < static_cast<unsigned int>(F.Nleft) && dist<bestDist1){
                                bestDist2=bestDist1;
                                bestDist1=dist;
                                bestIdxF=realIdxF;
                            }
                            else if(realIdxF < static_cast<unsigned int>(F.Nleft) && dist<bestDist2){
                                bestDist2=dist;
                            }

                            if(realIdxF >= static_cast<unsigned int>(F.Nleft) && dist<bestDist1R){
                                bestDist2R=bestDist1R;
                                bestDist1R=dist;
                                bestIdxFR=realIdxF;
                            }
                            else if(realIdxF >= static_cast<unsigned int>(F.Nleft) && dist<bestDist2R){
                                bestDist2R=dist;
                            }
                        }
                    ...
                    if(bestDist1<=TH_LOW)
                    {
                        if(static_cast<float>(bestDist1)<mfNNratio*static_cast<float>(bestDist2))
                        {
                            ...
                            const cv::KeyPoint &kp =
                                    ...
                                    (realIdxKF >= static_cast<const unsigned int>(pKF->NLeft)) ? pKF -> mvKeysRight[realIdxKF - pKF -> NLeft]
            ...
            if(f1it->first == f2it->first)
            {
                for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
                {
                    ...
                    const cv::KeyPoint &kp1 = (pKF1 -> NLeft == -1) ? pKF1->mvKeysUn[idx1]
                                                                    : (idx1 < static_cast<const size_t>(pKF1->NLeft)) ? pKF1 -> mvKeys[idx1]
                                                                                             : pKF1 -> mvKeysRight[idx1 - pKF1 -> NLeft];

                    const bool bRight1 = (pKF1 -> NLeft == -1 || idx1 < static_cast<const size_t>(pKF1->NLeft)) ? false
                                                                                       : true;
                    ...
                    for(size_t i2=0, iend2=f2it->second.size(); i2<iend2; i2++)
                    {
                        ...
                        const cv::KeyPoint &kp2 = (pKF2 -> NLeft == -1) ? pKF2->mvKeysUn[idx2]
                                                                        : (idx2 < static_cast<const size_t>(pKF2->NLeft)) ? pKF2 -> mvKeys[idx2]
                                                                                                 : pKF2 -> mvKeysRight[idx2 - pKF2 -> NLeft];
                        const bool bRight2 = (pKF2 -> NLeft == -1 || idx2 < static_cast<const size_t>(pKF2->NLeft)) ? false
                                                                                           : true;
